...
def wrap_dphi(phi1, phi2):
    """Return delta phi in range [-pi, pi], works with scalars or Awkward Arrays."""
    dphi = phi1 - phi2
...
def get_conversion_vertex(mcp_pdg, mcp_energy, mcp_pt, mcp_vx, mcp_vy, mcp_vz):
    ...
    # indices of highest-energy selected e+ / e- (per event)
    idx_ep_max = ak.argmax(ak.where(ep_sel, E, -np.inf), axis=1, keepdims=True)
    idx_em_max = ak.argmax(ak.where(em_sel, E, -np.inf), axis=1, keepdims=True)

    # gather per-event scalars (option types -> may be None if missing)
    vx_ep = ak.firsts(vx[idx_ep_max])
    vy_ep = ak.firsts(vy[idx_ep_max])
    vz_ep = ak.firsts(vz[idx_ep_max])
...
vertex_info = get_conversion_vertex(new_pdg, new_energy, new_pt, new_vx, new_vy, new_vz)

fig, ax = plt.subplots(figsize=(10, 6))
ax.scatter(ak.to_numpy(vertex_info["z"]), ak.to_numpy(vertex_info["r"]), s=6, zorder=2)
...
ax.set_ylim(0, 2500)
ax.set_title(rf"Photon conversion vertices (highest-energy $e^+e^-$ pair, {yn})")
fig.tight_layout()
plt.show()