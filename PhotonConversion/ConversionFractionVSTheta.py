...
for i in range(nbins):
    ...
    sel = (theta >= lo) & ((theta < hi) if i < nbins - 1 else (theta <= hi))
...
ax.stairs(fracs, edges)
ax.scatter(centers, fracs, s=12, zorder=3)
ax.set_xlabel(r"$\theta$ [rad]")
ax.set_ylabel("conversion fraction")
ax.set_xlim(0, np.pi)
ax.set_ylim(0,0.16)
ax.set_title(rf"Photon conversion fraction vs true $\theta$ ({yn})")
fig.tight_layout()
plt.show()
